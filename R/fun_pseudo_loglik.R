...
rc_copula_log_lik_eta <-function(p, p2, x1, x2,indata1,indata2, quantiles = NULL, copula, m.dist)
{
  ...
  if (m.dist == "Piecewise") {
    ...
    if (copula != "Copula2") {
      ...
      eta <- p
      beta<-p2[(n.cons+1):length(p2)] # coefficients
      ...
      for (i in 1:n.cons)
      {
        Lambda1<-Lambda1+ p2[i] * apply(cbind(rep(0,length(t1)), apply(cbind(rep(quantiles[i+1],length(t1)),t1),1,min) - quantiles[i]) ,1, max)
        lambda1<-lambda1+p2[i]*(t1<=quantiles[i+1])*(t1>quantiles[i])

        Lambda2<-Lambda2+p2[i]*apply(cbind(rep(0,length(t2)),apply(cbind(rep(quantiles[i+1],length(t2)),t2),1,min)-quantiles[i]),1,max)
        lambda2<-lambda2+p2[i]*(t2<=quantiles[i+1])*(t2>quantiles[i])
      }
